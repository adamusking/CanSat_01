...
Serial.println(adc0);
  ...
  void writeDataToSD() {
    if (SDinitialized==1){
      String timestamp = getTimestamp(); 
      int dotIndex = timestamp.lastIndexOf('.');  // Find the last dot in the filename
    String timestampWithoutExtension = timestamp.substring(0, dotIndex);
    File myFile = SD.open("/data.csv", FILE_APPEND);  // Leading slash required
  
    if (myFile) {
      myFile.print(timestampWithoutExtension);
      myFile.print(",");
